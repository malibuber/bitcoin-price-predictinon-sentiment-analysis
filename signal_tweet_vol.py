...
def signal1_tweet_vol():
    ...
    if(now.strftime("%A") == "Sunday") and (now.strftime("%H:%M") == "00:00"):
        ...
        model.save_weights("model_signal1.h5")
        ...
        json_file = open('model_signal1.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        model = model_from_json(loaded_model_json)
        # load weights into new model
        model.load_weights("model_signal1.h5")
...
def signal2_tweet_vol():
    ...
    #acc = met2.binary_accuracy(y_test,keras.backend.round(yhat),threshold=0.5)
        ...
        # save model
        model_json = model.to_json()
        with open("model_signal2.json", "w") as json_file:
            json_file.write(model_json)
        # serialize weights to HDF5
    ...
    #print(acc)
        ...
        from tensorflow.keras.models import model_from_json
        json_file = open('model_signal2.json', 'r')
        loaded_model_json = json_file.read()
        json_file.close()
        model = model_from_json(loaded_model_json)
        # load weights into new model
        model.load_weights("model_signal2.h5")
